...
const BAD_TOKEN_MESSAGE =
  "That token isn't valid. Generate a new one at https://afstim.com/settings/cli";

type Whoami = { email: string; name: string | null };
...
export async function loginCommand(): Promise<void> {
  ...
  try {
    token = (
      await promptMasked("Paste your token from https://afstim.com/settings/cli: ")
    ).trim();
  } catch (error) {
  ...
  let whoami: Whoami;
  try {
    whoami = await apiGet<Whoami>(apiUrl, WHOAMI_PATH, token);
  } catch (error) {
